...
from codebase.data import Dataset, create_loader, resolve_data_config, FastCollateMixup, mixup_target
from codebase.models import create_model, resume_checkpoint
from codebase.utils import *
from codebase.loss import LabelSmoothingCrossEntropy, SoftTargetCrossEntropy, LearnableLabelSmoothingCrossEntropy
from codebase.optim import create_optimizer
from codebase.scheduler import create_scheduler
...
def main():
    ...
    exp_name = '-'.join([socket.gethostname(),
            time_stamp,
            args.model,'logger.log'
        ])
    os.makedirs(os.path.join(output_base,'log'), exist_ok=True)
    logging.basicConfig(filename=os.path.join(output_base,'log',exp_name), filemode='w')
        ...
        for epoch in range(start_epoch, num_epochs):
            ...
            if not args.eval_only:
                train_metrics = train_epoch(
                   ...
                   use_amp=use_amp, model_ema=model_ema, grid = grid)
...
def train_epoch(
        ...
        lr_scheduler=None, saver=None, output_dir='', use_amp=False, model_ema=None, grid=None):
