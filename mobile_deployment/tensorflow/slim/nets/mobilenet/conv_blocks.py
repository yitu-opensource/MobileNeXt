...
def expanded_conv(input_tensor,
  ...
  with tf.variable_scope(scope, default_name='expanded_conv') as s, \
    ...
    if depthwise_location == 'input':
      ...
      net = depthwise_func(net, activation_fn=tf.nn.relu)
...
@slim.add_arg_scope
def i2rnet_conv(input_tensor,
                  num_outputs,
                  expansion_size=expand_input_by_factor(1/6),
                  stride=1,
                  rate=1,
                  kernel_size=(3, 3),
                  residual=True,
  ...
  Raises:
    TypeError: on inval
  """
  with tf.variable_scope(scope, default_name='i2rnet_conv') as s, \
       tf.name_scope(s.original_name_scope):
    prev_depth = input_tensor.get_shape().as_list()[3]
    if  depthwise_location not in [None, 'input', 'output', 'both']:
      raise TypeError('%r is unknown value for depthwise_location' %
                      depthwise_location)
    padding = 'SAME'
    if use_explicit_padding:
      padding = 'VALID'
    depthwise_func_1 = functools.partial(
        slim.separable_conv2d,
        num_outputs=None,
        kernel_size=kernel_size,
        depth_multiplier=depthwise_channel_multiplier,
        ...
        padding=padding,
        scope='depthwise_1')
    depthwise_func_2 = functools.partial(
        slim.separable_conv2d,
        num_outputs=None,
        kernel_size=kernel_size,
        depth_multiplier=depthwise_channel_multiplier,
    ...
    net = input_tensor
    if tensor_div > 0:
      sel_idx = prev_depth // tensor_div if prev_depth // tensor_div > 2 else 2
      partial_tensor = tf.slice(input_tensor, [0,0,0,0], [-1,-1,-1,sel_idx])

    # if depthwise_location == 'input':
    #   if use_explicit_padding:
    #     net = _fixed_padding(net, kernel_size, rate)
    if depthwise_location == 'input' or depthwise_location == 'both':
      net = depthwise_func_1(net, activation_fn=tf.nn.relu)
    net = tf.identity(net, name='depthwise_output_1')
    ...
    net = split_conv(
        ...
        scope='project',
        stride=1,
        normalizer_fn=normalizer_fn, activation_fn=None)
    # net = tf.identity(net, 'projection_output')
    if endpoints is not None:
      endpoints['projection_output'] = net
    ...
    if depthwise_location == 'output' or depthwise_location == 'both':
      net = depthwise_func_2(net, activation_fn=tf.nn.relu)
    net = tf.identity(net, name='depthwise_output_2')
    ...
    if callable(residual):  # custom residual
      ...
      # net += input_tensor
      if tensor_div > 0:
        net1, net2 = tf.split(net, [sel_idx, -1], -1)
        net1 = partial_tensor + net1
