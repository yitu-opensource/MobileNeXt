...
def main():
    ...
    with torch.cuda.device(0):
        flops, params = get_model_complexity_info(model, (3, args.input_size, args.input_size), as_strings=True, print_per_layer_stat=True)
        print("=>Flops:  " + flops)
        print("=>Params: " + params)
    ...
    val_loader, val_loader_len = get_val_loader(args.data, 100, workers=args.workers, input_size=args.input_size)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        print("Best Top-1 Acc: {}".format(best_prec1))
...
def train(train_loader, train_loader_len, model, criterion, optimizer, epoch):
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        # plot progress
        if (i + 1) % 100 == 0:
            info  = '({batch}/{size}) Data: {data:.3f}s | Batch: {bt:.3f}s | Loss: {loss:.4f} | top1: {top1: .4f} | top5: {top5: .4f}'.format(
                    batch=i + 1,
